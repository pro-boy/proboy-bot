...
async def save_welcome(event):
    ...
    if addwelcome_setting(event.chat_id, 0, string, msg_id) is True:
        return await event.edit(success.format("saved"))
    rmwelcome_setting(event.chat_id)
    if addwelcome_setting(event.chat_id, 0, string, msg_id) is True:
        return await event.edit( success.format("updated"))
    await event.edit("Error while setting welcome in this group")
...
async def show_welcome(event):
    cws = getcurrent_welcome_settings(event.chat_id)
    if not cws:
        await event.edit("`No pwelcome message saved here.`")
        return
    if cws.f_mesg_id:
        msg_o = await bot.get_messages(entity=BOTLOG_CHATID, ids=int(cws.f_mesg_id))
        await event.edit(
          "`I am currently pwelcoming new users with this welcome note.`"
        )
