...
async def add_new_filter(new_handler):
    ...
    if add_filter(str(new_handler.chat_id), keyword, string, msg_id) is True:
        return await new_handler.edit(success.format(keyword, "added"))
    remove_filter(str(new_handler.chat_id), keyword)
    if add_filter(str(new_handler.chat_id), keyword, string, msg_id) is True:
        return await new_handler.edit(success.format(keyword, "Updated"))
    await new_handler.edit(f"Error while setting filter for {keyword}")
...
async def on_all_snip_delete(event):
    ...
    if filters:
        ...
        await event.edit( f"There are no filters in this group")
